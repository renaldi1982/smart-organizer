...
public abstract class AbstractDataLoader<E extends List<?>> extends AsyncTaskLoader<E> {
	...
	/*
	 * Runs on UI thread, routing result from background thread 
	 * to whatever using the data list
	 */
	@Override 
	 ...
	 * the callbacks will be called on the UI thread. If previous load has been 
	...
	/*
	 * Must be called from UI thread triggered by a call to 
	 * stopLoading()
	 */
	@Override 
	...
	/*
	 * Must be called from the UI thread, trigger by a call to cancel(). Here
	...
	/*
	 * Must be called from the UI thread trigger by a call to reset(). Here
	 * we make sure our Cursor is closed, if it still exists and is not already 
	 * closed
	 */
	@Override
